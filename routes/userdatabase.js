...
/**
 * Visar ett tydligt rött felmeddelande vid nekad åtkomst
 */
...
router.get('/', async (request, response) => {
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <h2 style="font-size: 28px; font-weight: bold; color: #a8d5a8; margin: 0;">User Database</h2>
        <a href="/api/userdatabase/add" style="color: #ffffff; text-decoration: none; font-weight: bold; background-color: #3a7542; border: 1px solid #3a7542; padding: 10px 20px; border-radius: 5px; transition: background-color 0.3s; display: inline-block;">➕ Add New User</a>
    </div>
    <div id="table-resp">
        <div id="table-header">
...
router.get('/add', async (request, response) => {
    ...
    response.write(`
        <div class="page-section" style="max-width: 600px; margin: 2rem auto; padding: 2rem; background: rgba(13, 39, 23, 0.5); border: 1px solid rgba(58, 117, 66, 0.3); border-radius: 12px;">
            <h2 style="font-size: 28px; font-weight: bold; color: #a8d5a8; margin-bottom: 2rem; text-align: center; border-bottom: 2px solid #3a7542; padding-bottom: 1rem;">Add User &amp; Profile</h2>
            
            <form action="/api/userdatabase/add" method="POST" style="display: flex; flex-direction: column; gap: 1.5rem;">
                
                <!-- Username Selection -->
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    <label style="font-size: 12px; font-weight: bold; color: #a8d5a8; text-transform: uppercase; letter-spacing: 0.5px;">Username (Max 7 characters):</label>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; align-items: center;">
                        <div>
                            <input type="text" name="customUsername" maxlength="7" placeholder="Custom name..." style="width: 100%; padding: 10px; border: 1px solid #3a7542; background-color: rgba(13, 39, 23, 0.8); color: #d6e8be; border-radius: 4px; font-family: Arial, Helvetica, sans-serif; font-size: 13px; box-sizing: border-box;">
                            <p style="font-size: 11px; color: #a8d5a8; margin-top: 5px; text-align: center;">OR</p>
                        </div>
                        <select name="itUsername" style="width: 100%; padding: 10px; border: 1px solid #3a7542; background-color: rgba(13, 39, 23, 0.8); color: #d6e8be; border-radius: 4px; font-family: Arial, Helvetica, sans-serif; font-size: 13px;">
                ...
                <!-- Full Name -->
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    <label style="font-size: 12px; font-weight: bold; color: #a8d5a8; text-transform: uppercase; letter-spacing: 0.5px;">Full Name:</label>
                    <input type="text" name="fullName" placeholder="e.g., Thomas Anderson" style="width: 100%; padding: 10px; border: 1px solid #3a7542; background-color: rgba(13, 39, 23, 0.8); color: #d6e8be; border-radius: 4px; font-family: Arial, Helvetica, sans-serif; font-size: 13px; box-sizing: border-box;">
                </div>

                <!-- Password -->
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    <label style="font-size: 12px; font-weight: bold; color: #a8d5a8; text-transform: uppercase; letter-spacing: 0.5px;">Password (Max 7 characters):</label>
                    <input type="password" name="password" maxlength="7" required style="width: 100%; padding: 10px; border: 1px solid #3a7542; background-color: rgba(13, 39, 23, 0.8); color: #d6e8be; border-radius: 4px; font-family: Arial, Helvetica, sans-serif; font-size: 13px; box-sizing: border-box;">
                </div>

                <!-- Security Level -->
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    <label style="font-size: 12px; font-weight: bold; color: #a8d5a8; text-transform: uppercase; letter-spacing: 0.5px;">Security Clearance Level:</label>
                    <select name="securityLevel" style="width: 100%; padding: 10px; border: 1px solid #3a7542; background-color: rgba(13, 39, 23, 0.8); color: #d6e8be; border-radius: 4px; font-family: Arial, Helvetica, sans-serif; font-size: 13px;">
                        <option value="C">C (Standard - Read Only)</option>
                        <option value="B">B (Manager - Edit)</option>
                        <option value="A">A (Admin - Full Access)</option>
                    </select>
                </div>

                <!-- Action Buttons -->
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-top: 1rem;">
                    <button type="submit" style="padding: 12px 20px; font-size: 14px; font-weight: bold; background-color: #3a7542; color: #ffffff; border: 1px solid #3a7542; border-radius: 5px; cursor: pointer; text-transform: uppercase; letter-spacing: 0.5px; transition: background-color 0.3s; text-decoration: none;">Create User</button>
...
router.get('/delete/:username', async (request, response) => {
    ...
    try {
        // Radera från båda tabellerna för att hålla databasen ren
